package com.pheth.hasee.stickerhero.greendao;

// THIS CODE IS GENERATED BY greenDAO, EDIT ONLY INSIDE THE "KEEP"-SECTIONS

// KEEP INCLUDES - put your custom includes here
// KEEP INCLUDES END
/**
 * Entity mapped to table "FAVORITE_CATEGORY".
 */
public class FavoriteCategory {

    private Long id;
    private String title;
    private String url_full;
    private String url_thumb;
    private String identifier;
    private String url_full_local;
    private String url_thumb_local;
    private java.util.Date add_date;

    // KEEP FIELDS - put your custom fields here
    // KEEP FIELDS END

    public FavoriteCategory() {
    }

    public FavoriteCategory(Long id) {
        this.id = id;
    }

    public FavoriteCategory(Long id, String title, String url_full, String url_thumb, String identifier, String url_full_local, String url_thumb_local, java.util.Date add_date) {
        this.id = id;
        this.title = title;
        this.url_full = url_full;
        this.url_thumb = url_thumb;
        this.identifier = identifier;
        this.url_full_local = url_full_local;
        this.url_thumb_local = url_thumb_local;
        this.add_date = add_date;
    }

    public Long getId() {
        return id;
    }

    public void setId(Long id) {
        this.id = id;
    }

    public String getTitle() {
        return title;
    }

    public void setTitle(String title) {
        this.title = title;
    }

    public String getUrl_full() {
        return url_full;
    }

    public void setUrl_full(String url_full) {
        this.url_full = url_full;
    }

    public String getUrl_thumb() {
        return url_thumb;
    }

    public void setUrl_thumb(String url_thumb) {
        this.url_thumb = url_thumb;
    }

    public String getIdentifier() {
        return identifier;
    }

    public void setIdentifier(String identifier) {
        this.identifier = identifier;
    }

    public String getUrl_full_local() {
        return url_full_local;
    }

    public void setUrl_full_local(String url_full_local) {
        this.url_full_local = url_full_local;
    }

    public String getUrl_thumb_local() {
        return url_thumb_local;
    }

    public void setUrl_thumb_local(String url_thumb_local) {
        this.url_thumb_local = url_thumb_local;
    }

    public java.util.Date getAdd_date() {
        return add_date;
    }

    public void setAdd_date(java.util.Date add_date) {
        this.add_date = add_date;
    }

    // KEEP METHODS - put your custom methods here
    // KEEP METHODS END

}
